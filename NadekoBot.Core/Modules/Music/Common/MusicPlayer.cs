...
namespace NadekoBot.Modules.Music.Common
{
    ...
    public class MusicPlayer
    {
        ...
        // rewrite of the unsafe aidiakapi code
        public static byte[] AdjustVolume(byte[] audioSamples, float volume)
        {
            ...
            for (int i = 0; i + 1 < audioSamples.Length; i += 2) {
                short duplet = (short) (audioSamples[i] | (audioSamples[i+1] << 8));
                duplet = (short) (duplet * volumeFixed >> 16);
                audioSamples[i] = (byte) duplet;
                audioSamples[i+1] = (byte) (duplet >> 8);
            }
